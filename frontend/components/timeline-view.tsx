...
export function TimelineView({ period, startDate, swimlanes }: TimelineViewProps) {
  ...
  const getItemStyle = (start: Date, end: Date) => {
    ...
    switch (period) {
      ...
      case "weeks":
        startPosition = Math.max(0, differenceInCalendarWeeks(start, startDate))
      ...
      case "months":
        startPosition = Math.max(0, differenceInCalendarMonths(start, startDate))
    ...
    <div className="overflow-x-auto" ref={scrollContainerRef}>
      <div className="min-w-[800px]">
        ...
        <div className="flex border-b sticky top-0 bg-white z-10">
          <div className="w-48 min-w-48 border-r bg-gray-50 p-3 font-medium">Epics / Tasks</div>
          <div className="flex-1 grid" style={{ gridTemplateColumns: `repeat(${timeUnits.length}, 1fr)` }}>
            {timeUnits.map((unit) => {
